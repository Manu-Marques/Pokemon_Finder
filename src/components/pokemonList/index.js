...
export default function PokemonList({ pokemon }) {
    ...
    const searchPokemonName = useCallback(async (term) => {
      if (term) {
        const results = pokemons.current.filter(({name}) => (name === term));
        setFilterResults(results);
      } else {
        setFilterResults(pokemons.current);
      }
    }, [pokemons]);
    ...
    return (
        <div>
            ...
            <input type="text" value={term} onChange={(e) => setTerm(e.target.value)} id="search" className=" w-full p-4 pl-10 text-xl text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-50 dark:border-gray-600 dark:placeholder-gray-400 dark:text-gray-900 dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Pikachu, Pokemon version Platine ..." required />
