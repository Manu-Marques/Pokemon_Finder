            ...
            <form className="pt-2 pt-2 flex justify-center ">
                <div className="relative">
                    <div className="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
                        <svg aria-hidden="true" className="w-6 h-6 text-gray-500 text-gray-900" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path></svg>
                    </div>
                    <input value={inputValue} onChange={fusionFonction} aria-label="Nompokemon" className="placeholder:text-lg min-[320px]:w-80 md:w-search p-4 pl-10 text-xl text-gray-900 border border-4 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500 bg-gray-70 border-red-700 placeholder-gray-400 text-gray-900 focus:ring-blue-500 focus:border-blue-500" placeholder="Pikachu, Dracaufeu..." required />
                    <button onClick={resetInputField} className="text-white absolute right-2.5 bottom-4 bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-4 py-2 bg-blue-600 hover:bg-blue-700 focus:ring-blue-800">Effacer</button>
                </div>
            </form>
            {/* End Search Bar */}
            {/* Start Pokemon List */}
                ...
                {searchPokemon.length > 1 ? (
                    filterResults.map((poke) => {
                        return (
                            ...
                            <div className="pt-4 md:pt-40 flex flex-wrap justify-center" key={poke.id} >
                                <img className="bg-white border border-gray-200 rounded-lg shadow-md bg-gray-800 border-gray-700 border-4 w-96 rounded-lg" src={poke.image} alt="pokemon" />
                                <table className="h-96 w-96 sm:w-96 border-separate border border-slate-500 ...">
                                    <thead>
                                    </thead>
                                    <tbody>
                                        <tr>
                                            <td className=" w-96 text-gray-50 bg-bluekemon text-center border border-slate-600 ...">Nom</td>
                                        ...
                                        <tr>
                                            <td className="text-gray-50 bg-bluekemon text-center border border-slate-700 ...">Type</td>
                                        ...
                                        <tr>
                                            <td className="h-20 text-gray-50 bg-bluekemon text-center border border-slate-600 ...">Stats</td>
                    ...
                    pokemon?.map((poke) => {
                        return (
                            ...
                            <div key={poke.id} className="pt-4 pr-2 pl-2 pb-2 bg-grey-50 text-center text-4xl font-bold ">
                                <div className="max-w-sm bg-white border border-gray-200 rounded-lg shadow-md bg-gray-800 border-gray-700 inline-block align-middle">
                                    <div className="p-5">
                                        <Link to={`/details-card/${poke.name}`}>
                                            <img className="rounded-t-lg" src={poke.image} alt="" />
                                        </Link>
                                        <h5 className="mb-2 font-normal font-bold tracking-tight text-gray-900 underline text-white">{poke.name}</h5>
                                        <ul className="space-y-1 text-xs list-none list-inside text-gray-500 text-gray-400">
                                        </ul>
                                    </div>
                                </div>
                            </div>
